...
public class JewelMover : MonoBehaviour
{
    ...
    private IEnumerator MoveJewel(int BondEnergy)
    {
        ...
        if(BondMaker.MoleculeJustCompleted == true && DieScript.totalRolls < 8)
        {
            if (GameObject.Find("TutorialMarker").GetComponent<TutorialScript>().Tutorial == true && DieScript.totalRolls == 3)
            {
                GameObject.Find("ConversationDisplay").GetComponent<ConversationTextDisplayScript>().MoleculeCompletionTutorial();
