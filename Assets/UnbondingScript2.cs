...
public class UnbondingScript2 : MonoBehaviour
{
    ...
    //This script is attached to the FLAME that initiates UNBONDING EVENTS
    ...
    void OnTriggerEnter2D(Collider2D collider)
    {
...

        {
            ...
            /*if (collider.transform.root.gameObject.tag == "Diatomic")  //This section breaks a diatomic element into two atoms
            {
                ...
                if (Diatomic.GetComponent<DiatomicScript>().BondDissociationEnergy > DisplayCanvasScript.JouleTotal) 
                {
                ...
                { //This starts the diatomic dissociation process.  Calls GoldenJewelMover script that is attached to the GoldenJewelsToMove prefab             
                    DisplayCanvasScript.JouleTotal -= Diatomic.GetComponent<DiatomicScript>().BondDissociationEnergy;  //BDE is a public defined in DiatomicScript
                    DisplayCanvasScript.BonusPointTotal -= 10;   //diatomic molecule = 10 bonus pts, need to subtract when molecule is destroyed
            ...
            else if (DotCount == 1)  //indicates we have a second trigger (two unbonding triggers)
            {
                ...
                if (Atom1 != Atom2)
                {
                    ...
                    if (JouleCost > DisplayCanvasScript.JouleTotal)
                    {
                    ...
                    DisplayCanvasScript.JouleTotal -= JouleCost;
                    GameObject.Find("JouleHolder").GetComponent<JouleHolderScript>().AdjustJoulesInCorral();
    ...
    private void MCTokenSearch()  //If a MoleculeCompletionToken is in this list, remove it and decrement pt value
    {
        foreach (GameObject MCToken in AtomInventory.MoleculeList[MolIDValue])
        {
            ...
            if (MCToken.tag == "MCToken")         //Unbonding removes the COMPLETED MOLECULE TOKEN from the MoleculeList 
            {
                ...
                DisplayCanvasScript.BonusPointTotal -= MCToken.GetComponent<MoleculePtValues>().BonusPtValue;  //Value of MCToken subtracted from score
                break;
            }
        }
    }
